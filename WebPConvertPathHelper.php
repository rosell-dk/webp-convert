...
class WebPConvertPathHelper
{
    ...
    /* Get relative path between one dir and the other.
       ie
          ...
          result: ../../uploads/
       */
    public static function getRelativeDirectory($from_dir, $to_dir)
    {
    ...
    public static function abspath($relative_or_absolute_path, $root = null)
    {
        ...
        $relative_or_absolute_path = self::replaceBackslashes($relative_or_absolute_path);
        if ((substr($relative_or_absolute_path, 0, 1) == '/')) {
            return $relative_or_absolute_path;
        } else {
            return self::canonicalize(self::removeDoubleslash($root . '/' . $relative_or_absolute_path));
        }
    }
