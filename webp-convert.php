 ...
 * If set (if "&serve-image" is appended to the URL), the converted image
 * will be served. Otherwise the script will produce text output about the
 ...
 * Usually, you will not need to supply anything. Might be relevant in rare
 * occasions where the converter that generates the URL cannot pass all of
...
/*
 * $source:
 * Path to source file. Can be absolute or relative (relative to document
 * root). If it starts with "/", it is considered an absolute path.
*/
 ...
 * If you want converted files to be put in the same folder as the originals,
 * you can set destination-root to ".", or leave it blank. If you on the other
 * hand want all converted files to reside in their own folder, set the
 * destination-root to point to that folder. The converted files will be
 * stored in a hierarchy that matches the source files. With destination-root
 * set to "webp-cache", the source file "images/2017/cool.jpg" will be stored
 * at "webp-cache/images/2017/cool.jpg.webp". Both absolute paths and relative
 * paths are accepted (if the path starts with "/", it is considered an
 * absolute path). Double-dots in paths are allowed, ie "../webp-cache"
*/
$destination = WebPConvertPathHelper::get_destination_path($source, isset($_GET['destination-root'])
  ? $_GET['destination-root']
  : '.', $root);

/*
 * TODO: destination (optional)
 * Path to destination file. Can be absolute or relative (relative to document
 * root). You can choose not to specify destination. In that case, the path
 * will be created based upon source, destination-root and root-folder settings.
 * If all these are blank, the destination will be same folder as source, and
 * the filename will have ".webp" appended to it (ie image.jpeg.webp).
*/
...
/*
 * strip-metadata:
 * If set (if "&strip-metadata" is appended to the url), metadata will not be
 * copied over in the conversion process. Note however that not all converters
 ...
 * Setting this manipulates the default order in which the converters are
 * tried. If you for example set it to "cwebp", it means that you want "cwebp"
 * to be tried first. You can specify several favourite converters. Setting it
 * to "cwebp,imagewebp" will put cwebp to the top of the list and imagewebp
 ...
 * Default: "yes". Decides what action to take in the situation that (1) all
 * converters fails to convert the image, and (2) WebPConvert is told to serve
 * the converted image. the original image. Default action is to serve the
 * *original* image. End-users will not notice the fail, which is good on
